...
var Ringcheck = cmdutil.Commands{
	Name:        "ringcheck",
	Description: "gets the amount of rings of a player",
...
func ringcheckHandler(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	if err != nil {
		...
		cmdutil.SendInteractionResponse(s, i, cmdutil.CreateEmbed("⚠️ | **Warning**", "An error occurred while retrieving the user.", 0xffcc4d))
		return
	}
	...
	if err != nil {
		...
		cmdutil.SendInteractionResponse(s, i, cmdutil.CreateEmbed("⚠️ | **Warning**", "An error occurred while retrieving the player rings.", 0xffcc4d))
		return
	}

	switch len(playerRings) {
	case 0:
		cmdutil.SendInteractionResponse(s, i, &discordgo.MessageEmbed{
			Title:       "lmao.",
